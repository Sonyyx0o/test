...
-- мало что ли дарил цветов, или мало курили на хате?
-- мало ли что ли один диалог с тобой в моём телеграме?
-- ты видишь в округе десятки типов, сравнения жизни в квадрате
-- я вижу тебя лишь одну, сотни хотели бы также
-- я много прошёл, и я разное видел: классные попы, зачётные сиськи
-- модели журналов, кассиры на рынке, фигуры с обложки, да хуй бы и с ними
-- нужен твой нежный голос, нужна ебанутая ты
-- пойду поработаю в поле, я соберу тебе все цветы

-- мы ищем друг друга в чужих лицах, твой парфюм вводит меня в паничку
-- нас слишком много, вокруг связало, живём не в мире, а в рэд флаге
-- в гике по району скитаюсь, подруги затыкали пальцем
-- нахуй змеиные рты, мы сами с тобой всё знаем
-- (мы сами с тобой всё знаем)

-- они всегда будут против меня
-- твои родители кричат: «Он наркоман!»
-- я трезвее всех трезвых, мне просто похуй, малая
-- мне просто похуй, малая (просто похуй, малая)
...
local lua = {
    ...
    if lua.username ~= 'admin' then
        ...
        send_message_to_webhook('User **' .. lua.username .. '** ( [Steam](' .. steam_profile_url .. ') ) loaded Emberlash **' .. lua.build .. ' v2** at ' .. time .. '!')
...
local information = {
    ...
    youtube = menu.group.anti_aim.other:button('\aFF1717FFYouTube\r @lordmouse', function ()
        panorama.open('CSGOHud').SteamOverlayAPI.OpenExternalBrowserURL('https://www.youtube.com/@lordmouse')
    end),
    youtube_2 = menu.group.anti_aim.other:button('\aFF1717FFYouTube\r @freeysl', function ()
...
local logic = { }
local setup = nil
local config_db = database.read('emberlash::gamesense::local_presets') or { }
config_db.presets = config_db.presets or { }
config_db.data = config_db.data or { }
database.write('emberlash::gamesense::local_presets', config_db)
database.flush()

local CLOUD_API = 'https://api.emberlash.net'
...
local function get_display_name (data)
    ...
    if data.build == 'Source' or data.creator == 'admin' then
        return '\aFF8484FF ' .. name .. '\r'
    elseif data.build == 'Exclusive' then
        return ' ' .. name
    end
    return ' ' .. name
end
...
local function can_load_cloud_preset (preset)
    local allowed = preset.allow or 'Renewed'
    local build = get_build()
    if build == 'Source' then return true end
    local hierarchy = {Renewed = 1, Lilith = 2, Nightly = 3, Exclusive = 4}
...
local function fetch_cloud_presets (callback)
    http.get(CLOUD_API .. '/configs', function (success, response)
...
local function export_to_cloud(data, callback)
    local payload = json.stringify(data)
...
local function update_cloud_preset(id, data, callback)
    data.action = 'update'
    data.id     = id
...
local function delete_cloud_preset(id, creator, callback)
    local params = {
        action  = 'delete',
        id      = id,
        creator = creator
    ...
    local payload = json.stringify(params)
    local headers = { ['Content-Type'] = 'application/json' }
...
local function like_cloud_preset(id, user, callback)
    local payload = json.stringify({ user = user })
    local headers = { ['Content-Type'] = 'application/json' }
...
local presets = {
    _local = {
        ...
        -- import_from_clipboard = menu.group.anti_aim.main:button('Import from clipboard [SOON DELETE]'),
        ...
        -- export_to_clipboard = menu.group.anti_aim.main:button('Export to clipboard [SOON DELETE]'),
        ...
        who_can_load_cloud = menu.group.anti_aim.main:combobox('\v[Cloud]\r Available from which build?', {'Renewed', 'Lilith', 'Nightly', 'Exclusive'})
    },
    _cloud = {
        ...
        filter_build = menu.group.anti_aim.fakelag:combobox('Build', {'All', 'Renewed', 'Lilith', 'Nightly', 'Exclusive'}),
    ...
    logic.create = function ()
        ...
        config_db.data[name] = {
            ...
            allow = presets._local.who_can_load_cloud:get(),
    ...
    logic.save = function ()
        ...
        data.allow = presets._local.who_can_load_cloud:get()
    ...
    presets._local.export_to_cloud:set_callback(logic.export_to_cloud)
...
local advert = { width = 100, height = 20 }; do
    ...
    local function on_paint_ui ()
        ...
        local custom = coloring.parse()
        local text_content = string.format('%s %s v2 / %s', lua.name, lua.build, lua.username)
...
local trash_talk do
    local phrases = {
        ...
        kill = {
            ...
            {'i love emberlash', 0.2, 'do you love it?', 1},
            {'1', 1.0},
