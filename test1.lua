...
protected.database = {
	configs = ':Notchra::configs:'
}

local information = { user = database.read('angelbase_wtf^^') ~= nil and database.read('angelbase_wtf^^').username or fromdb.username }
local group = ui_handler.group('AA', 'Anti-aimbot angles')


local tab = group:combobox('\vnocthra\r', {'Home', 'Misc', 'AA'})
group:label(' ')
...
local Vars = {
    Home = {
		group:label('[\vnocthra\r] Information'),
		group:label('User: \v' .. information.user),
		group:label('Version: \v0.1 test'),
		group:label(' '),
		group:label('[\vnocthra\r] Presets'),
    ...
    Misc = {
        group:label('[\vnocthra\r] Visuals'),
...
Vars.Misc.show_hitrate:set_callback(function()
    Vars.Misc.reset_hitrate:set_visible(Vars.Misc.show_hitrate:get())
end)
...
local function update_hitrate_display()
    local hit_percentage = 0
    if hitrate_data.shots > 0 then
        hit_percentage = math.floor((hitrate_data.hits / hitrate_data.shots) * 100)
        if hit_percentage > 100 then
            hit_percentage = 100
        end
    end
    return string.format("%d/%d (%d%%)", hitrate_data.hits, hitrate_data.shots, hit_percentage)
end

local function simple_predict_movement(player_entity)
    if not entity.is_alive(player_entity) then return end
    
    local current_vel = {entity.get_prop(player_entity, "m_vecVelocity")}
    local current_pos = {entity.get_prop(player_entity, "m_vecOrigin")}
    
    local prediction_time = 0.15
    
    local predicted_pos = {
        current_pos[1] + current_vel[1] * prediction_time,
        current_pos[2] + current_vel[2] * prediction_time,
        current_pos[3] + current_vel[3] * prediction_time
    }
    
    return predicted_pos
end

local function analyze_player_behavior(player_entity)
    if not entity.is_alive(player_entity) then return "normal" end
    
    local current_angles = {entity.get_prop(player_entity, "m_angEyeAngles")}
    if not current_angles[1] then return "normal" end
    
    if not resolver_data.player_patterns[player_entity] then
        resolver_data.player_patterns[player_entity] = {
            angles_history = {},
            last_analysis = 0,
    ...
    local data = resolver_data.player_patterns[player_entity]
    
    table.insert(data.angles_history, {
        yaw = current_angles[1],
        pitch = current_angles[2],
        time = globals.curtime()
    })
    
    while #data.angles_history > 0 and globals.curtime() - data.angles_history[1].time > 3.0 do
        table.remove(data.angles_history, 1)
    end
    
    if #data.angles_history >= 5 then
        local yaw_changes = 0
        local max_yaw_change = 0
        local defensive_moves = 0
        
        for i = 2, #data.angles_history do
            local yaw_diff = math.abs(data.angles_history[i].yaw - data.angles_history[i-1].yaw)
            if yaw_diff > 5 then yaw_changes = yaw_changes + 1 end
            max_yaw_change = math.max(max_yaw_change, yaw_diff)
            
            if i > 2 then
                local prev_diff = data.angles_history[i-1].yaw - data.angles_history[i-2].yaw
                local curr_diff = data.angles_history[i].yaw - data.angles_history[i-1].yaw
                if prev_diff * curr_diff < 0 and math.abs(prev_diff) > 20 and math.abs(curr_diff) > 20 then
                    defensive_moves = defensive_moves + 1
                end
            end
        end
        
        if defensive_moves > 2 then
            data.behavior_type = "defensive"
            data.defensive_counter = defensive_moves
        elseif max_yaw_change > 60 then
            data.behavior_type = "jitter"
        elseif yaw_changes / #data.angles_history > 0.7 then
            data.behavior_type = "shaker" 
        elseif max_yaw_change < 15 then
            data.behavior_type = "static"
        else
            data.behavior_type = "normal"
        end
    end
    
    return data.behavior_type
end

local function smart_resolve_player(player_entity)
    if not entity.is_alive(player_entity) then return end
    
    local behavior_type = analyze_player_behavior(player_entity)
    local current_angles = {entity.get_prop(player_entity, "m_angEyeAngles")}
    if not current_angles[1] then return end
    
    local resolver_angles = {
        defensive = {58, -58, 62, -62, 55, -55, 65, -65},
        jitter = {0, 12, -12, 25, -25, 8, -8},
        shaker = {22, -22, 35, -35, 18, -18},
        static = {52, -52, 58, -58, 48, -48},
        normal = {0, 28, -28, 14, -14, 32, -32}
    }
    
    local angles = resolver_angles[behavior_type] or resolver_angles.normal
    
    if behavior_type == "defensive" then
        local data = resolver_data.player_patterns[player_entity]
        if data and data.defensive_counter > 3 then
            angles = {68, -68, 72, -72, 65, -65, 75, -75}
        end
    end
    
    local time_based_index = (globals.tickcount() % #angles) + 1
    local selected_angle = angles[time_based_index]
    
    local random_factor = (math.random() - 0.5) * 6
    selected_angle = selected_angle + random_factor
    
    entity.set_prop(player_entity, "m_angEyeAngles", selected_angle, current_angles[2])
end

client.set_event_callback("run_command", function()
    if not Vars.Misc.smart_resolver:get() and not Vars.Misc.prediction:get() then
        return
    end
    ...
    for i = 1, #players do
        ...
        if Vars.Misc.smart_resolver:get() then
            smart_resolve_player(player_entity)
        end
        
        if Vars.Misc.prediction:get() then
            simple_predict_movement(player_entity)
        end
    end
end)

-- Считаем попадания aimbot'а
client.set_event_callback("aim_hit", function(e)
    hitrate_data.hits = hitrate_data.hits + 1
    hitrate_data.shots = hitrate_data.shots + 1
end)

-- Считаем промахи aimbot'а
client.set_event_callback("aim_miss", function(e)
    hitrate_data.shots = hitrate_data.shots + 1
end)

-- Reset function остается такой же
Vars.Misc.reset_hitrate:set_callback(function()
    hitrate_data.hits = 0
end)
...
Vars.Misc.reset_hitrate:set_callback(function()
    hitrate_data.hits = 0
    hitrate_data.shots = 0
    hitrate_data.last_shot_time = 0
    hitrate_data.last_weapon_fire = 0
end)

client.set_event_callback("paint", function()
    if Vars.Misc.show_hitrate:get() then
        local hitrate_text = update_hitrate_display()
        renderer.indicator(255, 255, 255, 255, hitrate_text)
    end
end)
...
shot_logger.on_aim_hit = function(e)
	...
	shot_logger.add({ info.prefix[1], info.prefix[2], info.prefix[3], '[nocthra]'}, 
					{ 134, 134, 134, ' » ' }, 
...
screen_indication.handle = function()
	...
	if not (indication_enable or Vars.Misc.branded_watermark:get()) then
		if Vars.Misc.alt_watermark.value == "Modern" then
			...
			render.text(center[1], y - 15, 255, 255, 255, 200, 'cb', 0, 'nocthra')
		end
	end
	...
	anim.name.move = animations.new('binds_move_name', indication_enable and not scope_based and -render.measure_text(nil, 'nocthra')*0.5 or 15)
	...
	if anim.name.alpha > 1 then
		if anim.name.glow > 1 then
			render.shadow(center[1]+1 + anim.name.move, center[2]+7, render.measure_text('b', 'nocthra')-1, 0, 10, 0, {accent_color[1], accent_color[2], accent_color[3], anim.name.glow}, {accent_color[1], accent_color[2], accent_color[3], anim.name.glow})
		end
		render.text(center[1] + string.format('%.0f', anim.name.move), center[2], 255, 255, 255, anim.main, 'b', 0, utils.animate_text(globals.curtime()*2, 'nocthra', accent_color[1], accent_color[2], accent_color[3], anim.main, accent_color[1], accent_color[2], accent_color[3], 150*(anim.main/255)))
...
death_spammer.phares = {
	...
	"АААААААААААААААААААААААААААААААААААААА", "что ты делаешь даун", "миндамаг не выбил", "найс с дамагом хуесос", "даблтап забыл отжать бля", "КАК В ХАЙДШОТСЫ СУКА", 
...
"ñƤüβ£ϯ ΨнӹƤь ϯ£ნя £ნ£ϯ j£§ɥ§","忍び 1 УПАЛ び忍", "THIS nigga is so retarded", "(◣◢) 𝕐𝕠𝕦 𝕒𝕨𝕒𝕝𝕝 𝕗𝕚𝕣𝕤𝕥? 𝕆𝕜 𝕝𝕖𝕥𝕤 𝕗𝕦𝕟 🙂", "𝕚 𝕒𝕞 𝕙𝕧𝕙 𝕨𝕚𝕟𝕟𝕖", "𝙢𝙖𝙠𝙚 𝙨𝙠𝙚𝙚𝙩 𝙣𝙤 𝙛𝙧𝙞𝙚𝙣𝙙𝙨", "ａｌｌ　ｆａ𝑚ｉｌｙ　ａｗａｌｌ　ロく河", "𝘄𝗵𝗲𝗻 𝗶 𝗽𝗹𝗮𝘆 𝗺𝗮𝗶𝗻 𝗶 𝗰𝗮𝗿𝗲 𝗳𝗼𝗿 𝘀𝗺𝗼𝗸𝗲", 
...
"Tиктокершу заставили снять это, видео успели сохранить... Выложили по ссылке в комментах -> vk.com/vampire", "ｔ．ｍｅ／ｋｅｙｂｉｎｄｉｎｇ", "(っ◔◡◔)っ ♥ t.me/keybinding ♥", "【﻿ｔ．ｍｅ／ｋｅｙｂｉｎｄｉｎｇ】", "˜”*°•.˜”*°• t.me/keybinding •°*”˜.•°*”˜", "ＭＡＶＥＲＩＣＫ ｔｏｕｒｎａｍｅｎｔ ｈｉｇｈｌｉｇｈｔｓ ｆｔ ｇａ𝑚𝑒𝑠𝑒𝑛𝑠𝑒．ｐｕｂ ／ ｓｋｅｅｔ．ｃｃ",
...
"₽вȁл гȫρтăнь Ŧвȫей ʍа₮еᎵн", "Аллах бабах чурки", "АХМАТ СИЛА Алла́ху А́кбар ЛЕТИ", "[̲̅Д̲̅о̲̅П̲̅р̲̅ы̲̅Г̲̅а̲̲л̲̅с̲̅я̲̅(ت)̲̅Д̲̅р̲̅У̲̅ж̲̅о̲̅Ч̲̅е̲̅К̲̅]", "я ᴇбᴀᴧ ᴛʙᴏю ʍᴀᴛь [◣_◢]", "𝐢 𝐬𝐦𝐨𝐤𝐞 𝐜𝐡𝐚𝐫𝐨𝐧 𝐚𝐧𝐝 𝐝𝐫𝐢𝐧𝐤 𝐜𝐨𝐥𝐚 𝐝𝐨𝐛𝐫𝐢𝐲", "—Если братишка рядом —То враги падают градом 🧢Z", "отсосал как 𝕥𝕖𝕕𝕪","ℳАℳКУ ЕБАЛ","𝙸<𝟹 𝙾𝙿𝙴𝚁𝙿𝙻𝚄𝙶 ☆","𝐟𝐮𝐤 𝟏𝟎𝟐","-𝒓𝒆𝒑 𝒄𝒉𝒆𝒂𝒕𝒔","CАСЁШ КАК 𝕆ℝ𝔻","𝕏עӥ☾𝕠⊂","Я бы после такого в игру не заходил *fipp,albenix,sheefu,borya,vitma,st1vaha,d4ssh*","AWpKINGNeededSmoke $ 𝐓𝐡𝐞𝐲 𝐰𝐚𝐧𝐭 𝐭𝐨 𝐛𝐞 𝐥𝐢𝐤𝐞 𝐦𝐞 𝐛𝐮𝐭 𝐈 𝐛𝐞𝐥𝐨𝐧𝐠 𝐭𝐨 𝐚 𝐟𝐚𝐦𝐢𝐥𝐲 𝐨𝐟 𝐊𝐈𝐍𝐆𝐒","⋆н⋆а⋆ ⋆к⋆о⋆л⋆е⋆н⋆и⋆","😱 В Башкирии женщина год спала с любовником и заразилась от него ВИЧ","vk.com/tryhard <-— лʏчωии свᴀдᴇҕныи фотоrᴘᴀф","vk.com/VAMPIRE <-— лʏчωии свᴀдᴇҕныи фотоrᴘᴀф","ᙐᗣᏦᑌᕽᎽᙁ ᙅᕼᚿθᙅᑌᏦᗣ","匚ㄖᗪ乇10 卂ㄥ山卂ㄚ丂 ㄖ几","я иrᴘᴀю с 𝔸𝔸ℝℕ𝔼.ℂ𝕃𝕌𝔹, ҕᴇrитᴇ","𝐬𝐥𝐨𝐰𝐞𝐝 + 𝐫𝐞𝐯𝐞𝐫𝐛","₴ⱠØ₩ɆĐ + ⱤɆVɆⱤ฿","丂ㄥㄖ山乇ᗪ + 尺乇ᐯ乇尺乃","(っ◔◡◔)っ ♥ slowed + reverb ♥","˜”°•.˜”°• slowed + reverb •°”˜.•°”˜","ｓｌｏｗｅｄ ＋ ｒｅｖｅｒｂ","”°•.˜vk.com/vampire”°•","♥vk.com/vampire♥","𝕧𝕜.𝕔𝕠𝕞/𝕧𝕒𝕞𝕡𝕚𝕣𝕖","ｖｋ．ｃｏｍ／ｖａｍｐｉｒｅ","ᴠᴋ.ᴄᴏᴍ/ᴠᴀᴍᴘɪʀᴇ","𝚟𝚔.𝚌𝚘𝚖/𝚟𝚊𝚖𝚙𝚒𝚛𝚎","ｖｋ．ｃｏｍ／ｖａｍｐｉｒｅ　リゃド","【﻿ｖｋ．ｃｏｍ／ｖａｍｐｉｒｅ】","▀▄▀▄▀▄   🎀  𝓋𝓀.𝒸☯𝓂/𝓋𝒶𝓂𝓅𝒾𝓇𝑒  🎀   ▄▀▄▀▄▀","𝕒𝕝𝕝 𝕗𝕒𝕞𝕚𝕝𝕪 𝕨𝕚𝕥𝕙 𝕦𝕤𝕖𝕣𝕟𝕒𝕞𝕖𝕤","ａｌｌ ｆａｍｉｌｙ ｗｉｔｈ ｕｓｅｒｎａｍｅｓ","FNAAAAAY В ЧАААТ (◣_◢)"
}
