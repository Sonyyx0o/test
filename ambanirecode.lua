...
local ctx = new_class()
    ...
    render_crosshair = function (self)
		...
		render = function(self)
			...
			renderer.text(self.ss.x/2, self.ss.y/2 * 1.99, 255, 255, 255, 255, "c-", 0, "")
      ...
      if style == "default" then
        ...
        local visual_size = self.ui.menu.vis.indicatorfont:get()
			  ...
			  local strike_w, strike_h = renderer.measure_text(size, func"ambani")
			  ...
			  renderer.text(self.ss.x/2 + ((strike_w + 2)/2) * scoped_frac, self.ss.y/2 + 20, 255, 255, 255, 255-(150*self.weapon_fraction), size .. "c", 0, func"ambani")
